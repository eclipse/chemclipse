 ...
 * Jan Holy - additional functionality and UI improvements
 *******************************************************************************/
...
import org.eclipse.chemclipse.chromatogram.xxd.process.supplier.pca.ui.chart2d.BiPlot;
...
public class BiPlotPage {

	private BiPlot biplot;
	//
	private PcaEditor pcaEditor;
	private Spinner spinnerPCx;
	private Spinner spinnerPCy;

	public BiPlotPage(PcaEditor pcaEditor, TabFolder tabFolder, FormToolkit formToolkit) {
		//
	...
	private void initialize(TabFolder tabFolder, FormToolkit formToolkit) {
		...
		biplot = new BiPlot(compositeScorePlot);
		biplot.setLayoutData(new GridData(GridData.FILL_BOTH));
		//
		//
	...
	public void update() {
		...
		if(results.isPresent()) {
			updateSpinnerPCMaxima(results.get().getPcaSettings().getNumberOfPrincipalComponents());
