...
public class QuantitationSignalEntryEditDialog extends AbstractEntryEditDialog {
	...
	protected void buttonPressed(int buttonId) {
		...
		if(buttonId == IDialogConstants.OK_ID) {
			try {
				double ion = decimalFormat.parse(getWidgetInput(KEY_SIGNAL)).doubleValue();
				...
				quantitationSignalEntryEdit.setQuantitationSignal(quantitationSignalMSD);
	...
	protected Control createDialogArea(Composite parent) {
		...
		boolean use = true;
		//
		IQuantitationSignal quantitationSignalMSD = quantitationSignalEntryEdit.getQuantitationSignal();
		 ...
		 * Text fields, ...
		 */
		createTextInput(composite, layoutData, KEY_SIGNAL, "TIC = 0", ion, true);
	...
	protected boolean validateInput() {
		...
		if(isValid) {
			 ...
			 */
			try {
				double value = decimalFormat.parse(getWidgetInput(KEY_SIGNAL)).doubleValue();
				if(value < 0) {
					setErrorMessage("Select a signal >= 0 (TIC = 0).");
					return false;
				}
			} catch(ParseException e) {
				setErrorMessage("The signal is not valid.");
				return false;
			}
			/*
			 * Relative Response
