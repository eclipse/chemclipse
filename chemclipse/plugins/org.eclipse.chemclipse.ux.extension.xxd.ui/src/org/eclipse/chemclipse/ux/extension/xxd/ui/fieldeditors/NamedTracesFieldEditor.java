...
public class NamedTracesFieldEditor extends FieldEditor {
	...
	private void createTableSection(Composite parent) {
		...
		composite.setLayoutData(gridData);
		//
		namedTracesListUI = new NamedTracesListUI(composite, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		setTableViewerInput();
	}
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(IDialogConstants.OK_ID == dialog.open()) {
					...
					NamedTrace namedTrace = namedTraces.extractNamedTrace(item);
					if(namedTrace != null) {
						namedTraces.add(namedTrace);
						setTableViewerInput();
					}
				}
			}
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(object instanceof NamedTrace) {
					...
					keySetEdit.addAll(namedTraces.keySet());
					NamedTrace namedTrace = (NamedTrace)object;
					keySetEdit.remove(namedTrace.getIdentifier());
					InputDialog dialog = new InputDialog(e.display.getActiveShell(), "Named Trace", "Edit the selected named trace.", namedTraces.extractNamedTrace(namedTrace), new NamedTraceInputValidator(keySetEdit));
					if(IDialogConstants.OK_ID == dialog.open()) {
						String item = dialog.getValue();
						NamedTrace namedTraceNew = namedTraces.extractNamedTrace(item);
						if(namedTraceNew != null) {
							namedTraces.remove(namedTrace);
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if(MessageDialog.openQuestion(e.display.getActiveShell(), "Named Trace(s)", "Do you want to delete the selected named trace(s)?")) {
					...
					for(Object object : structuredSelection.toArray()) {
						if(object instanceof NamedTrace) {
							namedTraces.remove(((NamedTrace)object).getIdentifier());
						}
					}
					setTableViewerInput();
				}
	...
	private void setTableViewerInput() {
		...
		namedTracesListUI.setInput(namedTraces.values());
	}
	...
	protected void doLoadDefault() {
		...
		namedTraces.loadDefault(entries);
		setTableViewerInput();
	}
	...
	protected void doStore() {
		...
		getPreferenceStore().setValue(getPreferenceName(), namedTraces.save());
	}
