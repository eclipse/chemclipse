...
public class SettingsPreferencesEditPage extends WizardPage {
	...
	private void createToolItemEdit(ToolBar toolBar) {
		...
		toolItem.setToolTipText("Edit the stored data for the selected processor.");
		...
		toolItem.setImage(ApplicationImageFactory.getInstance().getImage(IApplicationImage.IMAGE_EDIT, IApplicationImage.SIZE_16x16));
		//
	...
	private void createToolItemDelete(ToolBar toolBar) {
		...
		//
		toolItem.setEnabled(false);
		toolItem.setToolTipText("Removes the stored data for the selected processor and resets it to the defaults.");
		toolItem.setImage(ApplicationImageFactory.getInstance().getImage(IApplicationImage.IMAGE_DELETE, IApplicationImage.SIZE_16x16));
		//
	...
	private void createToolItemDeleteAll(ToolBar toolBar) {
		...
		ToolItem toolItem = new ToolItem(toolBar, SWT.PUSH);
		toolItem.setToolTipText("Removes all stored data and resets to the defaults.");
		toolItem.setImage(ApplicationImageFactory.getInstance().getImage(IApplicationImage.IMAGE_DELETE_ALL, IApplicationImage.SIZE_16x16));
		toolItem.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {

				if(MessageDialog.openConfirm(e.display.getActiveShell(), "Settings", "Would you like to reset the settings for all processors?")) {
	...
	private void createToolItemExpand(ToolBar toolBar) {
		...
		ToolItem toolItem = new ToolItem(toolBar, SWT.PUSH);
		toolItem.setToolTipText("Expand all processor items.");
		toolItem.addSelectionListener(new SelectionAdapter() {
		...
		toolItem.setImage(ApplicationImageFactory.getInstance().getImage(IApplicationImage.IMAGE_EXPAND_ALL, IApplicationImage.SIZE_16x16));
	}

	private void createToolItemCollapse(ToolBar toolBar) {

		ToolItem toolItem = new ToolItem(toolBar, SWT.PUSH);
		toolItem.setToolTipText("Collapse all processor items.");
		toolItem.addSelectionListener(new SelectionAdapter() {
		...
		//
		toolItem.setEnabled(true);
		toolItem.setImage(ApplicationImageFactory.getInstance().getImage(IApplicationImage.IMAGE_COLLAPSE_ALL, IApplicationImage.SIZE_16x16));
	}
	...
	private void createColumnAskSettings(TreeViewer treeViewer) {
		...
		column.getColumn().setText("Display Settings");
		column.setLabelProvider(new ColumnLabelProvider() {
	...
	private void resetSettings(Shell shell) {
		...
		if(MessageDialog.openConfirm(shell, "Settings", "Would you like to reset the settings for the selected processors?")) {
