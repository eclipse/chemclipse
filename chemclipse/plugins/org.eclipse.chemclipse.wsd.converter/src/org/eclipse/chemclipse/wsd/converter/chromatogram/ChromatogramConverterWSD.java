...
public class ChromatogramConverterWSD {
	...
	private static final Logger logger = Logger.getLogger(ChromatogramConverterWSD.class);
	private static final String EXTENSION_POINT = "org.eclipse.chemclipse.wsd.converter.chromatogramSupplier";
	...
	public static IProcessingInfo convert(final File file, final String converterId, final IProgressMonitor monitor) {
		 ...
		 * be returned or null.
		 */
		IChromatogramWSDImportConverter importConverter = getChromatogramImportConverter(converterId);
	...
	public static IProcessingInfo convert(final File file, final IChromatogramWSD chromatogram, final String converterId, final IProgressMonitor monitor) {
		 ...
		 * be returned or null.
		 */
		IChromatogramWSDExportConverter exportConverter = getChromatogramExportConverter(converterId);
	 ...
	 */
	private static IChromatogramWSDImportConverter getChromatogramImportConverter(final String converterId) {

		IConfigurationElement element;
		element = getConfigurationElement(converterId);
		IChromatogramWSDImportConverter instance = null;
		if(element != null) {
			try {
				instance = (IChromatogramWSDImportConverter)element.createExecutableExtension(Converter.IMPORT_CONVERTER);
	 ...
	 */
	private static IChromatogramWSDExportConverter getChromatogramExportConverter(final String converterId) {

		IConfigurationElement element;
		element = getConfigurationElement(converterId);
		IChromatogramWSDExportConverter instance = null;
		if(element != null) {
			try {
				instance = (IChromatogramWSDExportConverter)element.createExecutableExtension(Converter.EXPORT_CONVERTER);
			} catch(CoreException e) {
				logger.warn(e);
			}
		}
