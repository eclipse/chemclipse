...
public class ReportWriter4 {
	...
	public void generate(File file, boolean append, List<IChromatogram<? extends IPeak>> chromatograms, ReportSettings4 reportSettings, IProgressMonitor monitor) throws IOException {
	...
	private void reportChromatogram(PrintWriter printWriter, IChromatogram chromatogram, IProgressMonitor monitor) {
		...
		List<IPeak> peaks = new ArrayList<IPeak>(chromatogram.getPeaks());
	...
	private void reportPeak(PrintWriter printWriter, IPeak peak, int number, IProgressMonitor monitor) {
		...
		if(peakModel instanceof IPeakModelMSD) {
			...
			highestTraces = extractHighestIons(printWriter, peakModelMSD);
		} else {
