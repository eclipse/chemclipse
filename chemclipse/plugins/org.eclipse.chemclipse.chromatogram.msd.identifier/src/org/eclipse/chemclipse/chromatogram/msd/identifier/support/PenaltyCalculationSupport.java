...
public class PenaltyCalculationSupport {
     ...
     * @return float
     */
    public static float calculatePenaltyFromRetentionIndex(final IScanMSD unknown, final IScanMSD reference,
	    final float retentionIndexWindow, final float penaltyCalculationLevelFactor, final float maxPenalty) {
	...
	try {
	    ...
	    //
	    if (maxPenalty < IComparisonResult.MIN_ALLOWED_PENALTY
		    || maxPenalty > IComparisonResult.MAX_ALLOWED_PENALTY) {
		throw new IllegalArgumentException();
	    }
	    runRetentionIndexCheck(unknown, reference);
	    return calculatePenalty(unknown.getRetentionIndex(), reference.getRetentionIndex(), retentionIndexWindow,
		    penaltyCalculationLevelFactor, maxPenalty);
     ...
     * @return float
     */
    public static float calculatePenaltyFromRetentionTime(final int unknown, final int reference,
	    final int retentionTimeWindow, final float penaltyCalculationLevelFactor, final float maxPenalty) {
	...
	try {
	    ...
	    result = calculatePenalty(unknown, reference, retentionTimeWindow, penaltyCalculationLevelFactor,
		    maxPenalty);
	} catch (final Exception e) {
    ...
    public static float calculatePenalty(final float valueUnknown, final float valueReference, final float valueWindow,
	    final float penaltyCalculationLevelFactor, final float maxPenalty) {
