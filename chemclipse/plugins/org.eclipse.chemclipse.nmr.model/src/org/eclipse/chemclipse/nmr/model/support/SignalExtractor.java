...
public class SignalExtractor implements ISignalExtractor {
	...
	public void setPhaseCorrectedData(Complex[] phaseCorrectedData) {
		...
		while(signals.hasNext()) {
			...
			Complex phaseCorrection = phaseCorrectedData[i];
			signal.setPhaseCorrectedData(phaseCorrection);
			signal.setBaselineCorrectedData(phaseCorrection);
			signal.setIntensity(phaseCorrection.getReal());
		}
	}
	...
	public void setBaselineCorrectedData(Complex[] baselineCorrectedData) {
		...
		while(signals.hasNext()) {
			ISignalNMR signal = signals.next();
			Complex baselineCorection = baselineCorrectedData[i];
			signal.setBaselineCorrectedData(baselineCorection);
			signal.setIntensity(baselineCorection.getReal());
		}
	}
